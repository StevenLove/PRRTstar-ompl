...
remove_first_child(prrts_link_t *parent)
{
    ...
    } while (!__sync_bool_compare_and_swap(&parent->first_child, child
                                         , sibling));
...
remove_child(prrts_link_t *parent, prrts_link_t *child)
{
    ...
    for (;;) {
        ...
        if (n == child) {
            ...
            if (__sync_bool_compare_and_swap(&parent->first_child, child
                                           , sibling)) {
                break;
            } else {
                continue;
        ...
        for (;;) {
            ...
            if (n == child) {
                 ...
                 */
                sibling = child->next_sibling;
                if (__sync_bool_compare_and_swap(&p->next_sibling, child
                                               , sibling)) {
                    goto done;
                } else {
                    break;
...
worker_near_callback(void *worker_arg, int no, void *near_node, double dist)
{
    ...
    if (no == (int)worker->near_list_capacity) {
         ...
         */
        if ((worker->near_list 
               = (near_list_t*)realloc(worker->near_list
                                     , sizeof(near_list_t) * 2 * no)) == NULL){
            err(1, "failed to reallocate cost nodes (requested %lu bytes)"
          ...
          , prrts_link_t *parent)
{
    ...
    if ((new_link = (prrts_link_t*)local_alloc(&worker->link_heap
                                             , sizeof(prrts_link_t)
...
add_child_link(prrts_link_t *parent, prrts_link_t *child)
{
    ...
    do {
         ...
         * builtin atomic.
         */
        if (!__sync_bool_compare_and_swap(&child->next_sibling
                                        , expected, next_sibling)) {
            assert(false);
        }
        expected = next_sibling;

    } while (!__sync_bool_compare_and_swap(&parent->first_child
                                          , next_sibling, child));
}

static prrts_node_t *
create_node(worker_t *worker
          , double *config
          , size_t dimensions
          , bool in_goal
          , double link_cost
          , prrts_link_t *parent_link)
{
    ...
    if ((new_node = (prrts_node_t *)local_alloc(&worker->node_heap
                                              , sizeof(prrts_node_t)
                                              , CACHE_LINE_SIZE/2)) == NULL) {
...
static prrts_link_t *
set_node_link(worker_t *worker
            , prrts_node_t *node
            , prrts_link_t *old_link
            , double link_cost
            , prrts_link_t *parent_link)
{
    prrts_link_t *new_link;
